...
class BotManager:
    ...
    def __init__(self):
        ...
        self.risk_settings = {
            'maxLeverage': trading_cfg.get('max_leverage', 25),
            ...
            'symbolLeverage': trading_cfg.get('symbol_leverage', {
                'BTCUSDT': 15, 'ETHUSDT': 15, 'SOLUSDT': 10
            })
        }
...
def bot_analysis():
    ...
    When bot is running: return the SAME signals the bot
    uses for trading (from SmartBotEngine.latest_analysis).
    When bot is stopped: fallback to independent calculation
    using the same override_threshold as the bot (55).
    """
    try:
        symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT']
        analysis_data = {}
        ...
        # =============================================
        # PRIORITY: Use bot's actual analysis if running
        # This guarantees dashboard == trading signals
        # =============================================
        smart_bot = (
        ...
        # =============================================
        # FALLBACK: Bot not running → calculate directly
        # Uses same override_threshold=55 as the bot
        # =============================================
        import joblib
        import numpy as np
        ...
        for symbol in symbols:
            try:
                ...
                if symbol in models:
                    ...
                    # SAME override_threshold as bot (55)
...
def execute_signal():
    ...
    try:
        ...
        # Fallback: direct execution via binance_client
        if not binance_client:
            return jsonify({'success': False, 'error': 'Client not initialized'})
        ...
        try:
            ...
            return jsonify({'success': True, 'message': f'{side} order for {symbol} executed', 'order': order})
    ...
    try:
        ...
        for pos in positions:
            if float(pos['positionAmt']) != 0:
                ...
                position_side = 'LONG' if float(pos['positionAmt']) > 0 else 'SHORT'  # For Hedge Mode
        ...
        if binance_client.is_demo_mode:
            ...
            return jsonify({
                ...
                'data': {
                    ...
                    'spot': f'{binance_client.demo_account.balance:.6f} USDC',
                    'total': f'{binance_client.demo_account.balance:.6f} USDC',
                    'sufficient': True
                },
                'timestamp': datetime.now().strftime('%H:%M:%S')
            })
