...
class SmartBotEngine:
    ...
    def _momentum_filter(
        ...
        try:
            ...
            # Block counter-trend on moderate+ momentum
            if mom_str >= 25:
            ...
            # Block SHORT if 4+ green candles + rising
            if (signal == 'SHORT'
                    and consec_g >= 4
                    and short_pct > 0.3):
                logger.warning(
                    f"   🚫 BLOCKED SHORT {symbol}: "
                    f"{consec_g} green candles"
                )
                signal, confidence = 'HOLD', 0

            # Block LONG if 4+ red candles + falling
            if (signal == 'LONG'
                    and consec_r >= 4
                    and short_pct < -0.3):
                logger.warning(
                    f"   🚫 BLOCKED LONG {symbol}: "
                    f"{consec_r} red candles"
                )
                signal, confidence = 'HOLD', 0
    ...
    async def check_risk_before_trade(self, symbol, side):
        ...
        for pos in current_positions:
            if pos['symbol'] == symbol:
                logger.warning(
                    f"⚠️ Already has position in {symbol}"
                )
                return False, f"Already has {symbol}"
    ...
    async def execute_trade(self, signal_data):
        ...
        try:
            ...
            leverage = sym_lev.get(symbol, 15)
    ...
    async def place_stop_loss(
        ...
        try:
            ...
            # This ensures SL always closes the ENTIRE remaining
            # position — critical when partial TP has already
            # reduced the size (avoids flipping to opposite side)
            order = self.client.client.futures_create_order(
                symbol=symbol,
                side=sl_side,
                type='STOP_MARKET',
    ...
    async def emergency_close_all(self):
        ...
        for pos in positions:
            try:
                ...
                position_side = pos['side']  # LONG or SHORT for Hedge Mode
                quantity = pos['size']
